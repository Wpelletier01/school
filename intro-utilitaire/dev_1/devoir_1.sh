...
main() {  
  ...
  # Je l'ai laisser mais inutile car les arguments sont passé en guillemets
  # s'assure qu'un seul argument est passé
  if [ $# -le 0 -o $# -ge 2 ] 
  then

    echo "Vous avez besoin de passé un fichier minimun en argument"
    echo "Si le nom du fichier contient des espaces asurez vous de le mettre entre guillemets"
  ...
  # ajoute une ligne vide car si la derniere ligne contient du texte et qu'elle contient 
  # un 'break sign', elle ne sera pas compté par la commande wc -l 
  echo -en "\n" >> "$1"
